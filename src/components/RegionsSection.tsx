      ...
      <div className="container mx-auto px-4">
              ...
              >
                <Link to={r.link} className="flex flex-col h-full">
                  <div className="relative h-48 w-full overflow-hidden rounded-t-2xl shadow-md">
                    <img src={r.image} alt={`Réparation volets roulants ${r.name}`} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" loading="lazy" />
                    <div className="absolute inset-0 bg-black/5" />
                    {r.highlight && (
                      <div className="absolute top-3 right-3 z-10">
                        <Badge variant="serviceBlue" className="text-[10px] font-bold text-white shadow-lg">PRIORITAIRE</Badge>
                      </div>
                    )}
                  </div>
                  <div className={`flex-1 p-6 rounded-b-2xl border border-t-0 transition-all duration-500 card-shadow group-hover:card-shadow-hover ${r.color}`}>
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <h3 className={`text-lg font-bold ${r.highlight ? "text-service-blue" : "text-foreground"}`}>{r.name}</h3>
                    </div>
        ...
        <div className="mt-12 text-center flex flex-col sm:flex-row items-center justify-center gap-4">
          <Button onClick={() => setIsExpanded(!isExpanded)} variant="outline"
            ...
            {isExpanded ? (<>Voir moins <ChevronUp className="ml-2 h-5 w-5" /></>) : (<>Voir toutes les régions <ChevronDown className="ml-2 h-5 w-5" /></>)}
          </Button>
